...
export default function About() {
  ...
  return (
    <div
      className="relative text-text w-full snap-y snap-mandatory scroll-smooth overflow-y-auto h-screen"
      ref={scrollRef}
    >
      <div className="xl:h-[85vh] max-xl:h-screen snap-start">
        ...
        <div className="w-full h-full relative">
          ...
          >
            <div className="flex flex-col items-center gap-8 xl:w-1/5 px-4 py-8">
              ...
              <p className="max-w-sm text-center xl:mt-24">
          ...
          <motion.div
            className="flex xl:flex-row max-xl:flex-col-reverse
           w-full gap-8 items-center  max-xl:hidden"
            initial={{ opacity: 0 }}
            whileInView={{ opacity: 1 }}
            transition={{ duration: 0.2 }}
            viewport={{ once: true }}
          >
            <div className="xl:w-1/5 xl:ml-[40vw] relative bg-mainbg p-1.5 text-[1.2rem] border-0 outline-0 overflow-hidden max-xl:w-96 h-full rounded-3xl drop-shadow-lg xl:self-end aspect-[4/5]">
              <div className="w-full h-full rounded-[20px] flex justify-between items-center flex-col text-center text-white border border-border backdrop-blur-2xl bg-[url('/me.png')] bg-cover bg-center overflow-hidden">
                ...
                <div className="aboutme-bottom h-1/4 w-full flex flex-row items-end">
                  <div className="flex flex-row items-center w-full gap-2 p-4">
                    <p className="text-md text-white">Want to connect?</p>
                    <hr className="flex-1 border-white" />
                    <button className="flex py-2 px-3 border-[0.5px] transition-all border-blue bg-[radial-gradient(100%_100%_at_50%_100%,_#000E8A_0%,_#2A8DFF_100%)] rounded-full text-sm">
                      Contact
                    </button>
                  </div>
                </div>
      ...
      <motion.div
        className="flex xl:flex-row max-xl:flex-col w-full gap-8 items-end h-[80vh] transition-all duration-600 snap-start relative  max-xl:hidden"
        initial={{
          opacity: 0,
        }}
        animate={{
        ...
        transition={{ duration: 0.2 }}
        style={section === 1 ? { filter: "none" } : { filter: "blur(6px)" }}
      >
        <div
          className="xl:w-[calc(20vw-32px)] xl:ml-[20vw] flex flex-col gap-4 max-xl:text-center max-xl:px-4 transition-all h-2/3 relative justify-center"
          style={section === 1 ? { top: 0 } : { top: "-60%" }}
        >
          <h1 className={`${libre.className} text-4xl`}>
            Technologies I <br />
            work with
        ...
        <div className="xl:w-1/5 relative bg-mainbg p-1.5 text-[1.2rem] border-0 outline-0 overflow-hidden max-xl:w-96 rounded-3xl drop-shadow-lg self-end aspect-[181/236]">
      ...
      <motion.div
        className="flex xl:flex-row max-xl:flex-col-reverse w-full gap-8 items-center h-screen transition-all duration-600 snap-start mb-24 relative  max-xl:hidden"
        initial={{
          opacity: 0,
        }}
        animate={{
        ...
        transition={{ duration: 0.2 }}
        style={section === 2 ? { filter: "none" } : { filter: "blur(6px)" }}
      >
        <div className="xl:w-1/5 xl:ml-[40vw] relative bg-mainbg p-1.5 text-[1.2rem] border-0 outline-0 overflow-hidden max-xl:w-96 rounded-3xl drop-shadow-lg aspect-[181/236]">
          <div className="w-full h-full flex rounded-[20px] items-center justify-end flex-col text-center text-text p-2 border border-border backdrop-blur-2xl bg-(image:--gradient-mainbuttonbg)">
            <div className="flex flex-col gap-2 bg-lines w-full rounded-xl p-2">
              <p className="text-left text-sm">
                I'm a frontend developer with a passion for clean code, smooth
                animations, and building web experiences that actually feel good
                to use.
              </p>
              <p className="text-left text-sm">
                Whether it's turning a Figma design into a pixel-perfect layout,
                or debugging the weirdest edge case, I genuinely enjoy the
                challenge.
              </p>
              <p className="text-left text-sm">
        ...
        <div
          className="xl:w-[calc(20vw-32px)] flex flex-col gap-4 max-xl:text-center max-xl:px-4 transition-all h-2/3 relative justify-center"
          style={section === 2 ? { top: 0 } : { top: "-40%" }}
        >
          <h1 className={`${libre.className} text-4xl`}>
